...
public class CSharpDocumentationProvider implements LanguageDocumentationProvider {
    ...
    private static String generateQuickLikeMethodDeclarationInfo(DotNetLikeMethodDeclaration element, boolean isFullDocumentation) {
        ...
        if (element instanceof DotNetConstructorDeclaration constructorDeclaration) {
            if (constructorDeclaration.isDeConstructor()) {
                builder.append("~");
            }
            ...
            if (element instanceof CSharpIndexMethodDeclaration) {
                ...
                if (name != null) {
					XmlStringUtil.escapeText(name, builder);
				}
        ...
        builder.append(StringUtil.join(element.getParameters(), new Function<>() {
            @Override
            @RequiredReadAction
        ...
        if (isFullDocumentation) {
            ...
            if (vFile != null && (fileIndex.isInLibrarySource(vFile) || fileIndex.isInLibraryClasses(vFile))) {
                ...
                if (orderEntries.size() > 0) {
                    ...
                    builder.append("[");
                    XmlStringUtil.escapeText(orderEntry.getPresentableName(), builder);
                    builder.append("] ");
                }
            }
            else {
                Module module = containingFile == null ? null : containingFile.getModule();
                if (module != null) {
    ...
    private static String generateLinksForType(DotNetTypeRef typeRef, PsiElement element, boolean qualified) {
        ...
        if (typeRef == DotNetTypeRef.AUTO_TYPE) {
            ...
            if (dotNetTypeResolveResult.getElement() instanceof DotNetQualifiedElement qualifiedElem) {
                if (qualifiedElem instanceof DotNetGenericParameterListOwner parameterListOwner) {
                    wrapToLink(parameterListOwner, builder, qualified);
                    ...
                    if (genericParameters.length > 0) {
                        ...
                        XmlStringUtil.escapeText("<", builder);
                        for (int i = 0; i < genericParameters.length; i++) {
                            if (i != 0) {
                                builder.append(", ");
                            }
                    ...
                    builder.append(qualifiedElem.getName());
                }
            }
            else {
