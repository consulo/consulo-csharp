...
public class HidingOrOverridingElementCollector implements LineMarkerCollector {
    private static class OurHandler implements GutterIconNavigationHandler<PsiElement> {
        ...
        public void navigate(ComponentEvent<?> event, PsiElement element) {
            CSharpLineMarkerUtil.openTargets(event, element.getProject(), "Searching for overriding", CSharpLineMarkerUtil.BY_PARENT, () -> {
                ...
                return virtualImplementOwner == null ? List.of() : OverrideUtil.collectOverridingMembers(virtualImplementOwner);
            });
        }
    }
